...
function
promo(){echo'<a href="http://nette.org" title="Nette Framework - The Most Innovative PHP Framework"><img ','src="http://files.nette.org/icons/nette-powered.gif" alt="Powered by Nette Framework" width="80" height="15" /></a>';}}class
...
run(){$httpRequest=$this->getHttpRequest();$httpResponse=$this->getHttpResponse();$httpRequest->setEncoding('UTF-8');$session=$this->getSession();if(!$session->isStarted()&&$session->exists()){$session->start();}if($this->allowedMethods){$method=$httpRequest->getMethod();if(!in_array($method,$this->allowedMethods,TRUE)){$httpResponse->setCode(IHttpResponse::S501_NOT_IMPLEMENTED);$httpResponse->setHeader('Allow',implode(',',$this->allowedMethods));echo'<h1>Method '.htmlSpecialChars($method).' is not implemented</h1>';return;}}$request=NULL;$repeatedError=FALSE;do{try{if(count($this->requests)>self::$maxLoop){throw
new
...
__construct($dir,Context$context=NULL){if(self::$useDirectories===NULL){$uniq=uniqid('_',TRUE);umask(0000);if(!@mkdir("$dir/$uniq",0777)){throw
new
...
__construct($host='localhost',$port=11211,$prefix='',Context$context=NULL){if(!self::isAvailable()){throw
new
NotSupportedException("PHP extension 'memcache' is not loaded.");}$this->prefix=$prefix;$this->context=$context;$this->memcache=new
Memcache;Debug::tryError();$this->memcache->connect($host,$port);if(Debug::catchError($msg)){throw
new
InvalidStateException($msg);}}function
...
new
NotSupportedException('Dependent items are not supported by MemcachedStorage.');}$meta=array(self::META_DATA=>$data);$expire=0;if(isset($dp[Cache::EXPIRE])){$expire=(int)$dp[Cache::EXPIRE];if(!empty($dp[Cache::SLIDING])){$meta[self::META_DELTA]=$expire;}}if(isset($dp[Cache::CALLBACKS])){$meta[self::META_CALLBACKS]=$dp[Cache::CALLBACKS];}if(isset($dp[Cache::TAGS])||isset($dp[Cache::PRIORITY])){if(!$this->context){throw
new
InvalidStateException('CacheJournal has not been provided.');}$this->getJournal()->write($this->prefix.$key,$dp);}$this->memcache->set($this->prefix.$key,$meta,0,$expire);}function
...
new
FileNotFoundException("File '$file' is missing or is not readable.");}Debug::tryError();$ini=parse_ini_file($file,TRUE);if(Debug::catchError($msg)){throw
new
...
NULL;}else{if(self::$showBar){self::$errors[]="$message in ".(self::$editor?'<a href="'.htmlspecialchars(strtr(self::$editor,array('%file'=>rawurlencode($file),'%line'=>$line)))."\">$file:$line</a>":"$file:$line");}if(self::$firebugDetected&&!headers_sent()){self::fireLog(new
...
as$k=>&$v){if($k[0]==="\x00"){$k=substr($k,strrpos($k,"\x00")+1);}$res[self::fireDump($k)]=self::fireDump($v,$level+1);}array_pop($list);return$res;}else{return" \xE2\x80\xA6 ";}}elseif(is_resource($var)){return"resource ".get_resource_type($var);}else{return"unknown type";}}}Debug::_init();class
Configurator
extends
Object{public$defaultConfigFile='%appDir%/config.ini';public$defaultServices=array('Nette\\Application\\Application'=>array(__CLASS__,'createApplication'),'Nette\\Web\\HttpContext'=>'HttpContext','Nette\\Web\\IHttpRequest'=>'HttpRequest','Nette\\Web\\IHttpResponse'=>'HttpResponse','Nette\\Web\\IUser'=>'User','Nette\\Caching\\ICacheStorage'=>array(__CLASS__,'createCacheStorage'),'Nette\\Caching\\ICacheJournal'=>array(__CLASS__,'createCacheJournal'),'Nette\\Web\\Session'=>'Session','Nette\\Loaders\\RobotLoader'=>array(__CLASS__,'createRobotLoader'));function
...
Environment::getContext();$context->addService('Nette\\Application\\IRouter','MultiRouter');if(!$context->hasService('Nette\\Application\\IPresenterLoader')){$context->addService('Nette\\Application\\IPresenterLoader',callback(create_function('','
			...
			')));}$application=new
Application;$application->setContext($context);$application->catchExceptions=Environment::isProduction();return$application;}static
function
...
new
FileStorage($dir,$context);}static
function
createCacheJournal(){{return
new
FileJournal(Environment::getVariable('tempDir').'/cache');}}static
function
...
instanceof
Html){$dest->add((string)$value2->selected(isset($selected[$key2])));}else{$key2=$this->useKeys?$key2:$value2;$value2=$this->translate($value2);$dest->add((string)$option->value($key2===$value2?NULL:$key2)->selected(isset($selected[$key2]))->setText($value2));}}}return$control;}static
function
...
InvalidStateException("Ambiguous class '$class' resolution; defined in $file and in ".$this->list[$lClass][0].".");if(PHP_VERSION_ID<50300){Debug::_exceptionHandler($e);exit;}else{throw$e;}}$this->list[$lClass]=array($file,$time,$class);}private
function
...
function
getKey(){return
md5("v2|$this->ignoreDirs|$this->acceptFiles|".implode('|',$this->scanDirs));}}class
MailMimePart
extends
Object{const
...
initialize($filter,&$s){$this->filter=$filter;$this->blocks=array();$this->namedBlocks=array();$this->extends=NULL;$this->uniq=substr(md5(uniqid('',TRUE)),0,10);$this->cacheCounter=0;$filter->context=LatteFilter::CONTEXT_TEXT;$filter->escape='TemplateHelpers::escapeHtml';$s=String::replace($s,'#\\{\\*.*?\\*\\}[\r\n]*#s','');$s=String::replace($s,'#@(\\{[^}]+?\\})#s','<?php } ?>$1<?php if (SnippetHelper::$outputAllowed) { ?>');}function
finalize(&$s){if(count($this->blocks)===1){$s.=$this->macro('/block','','');}elseif($this->blocks){throw
new
InvalidStateException("There are unclosed blocks.");}$s="<?php\nif (".'SnippetHelper::$outputAllowed'.") {\n?>$s<?php\n}\n?>";if($this->namedBlocks||$this->extends){$s="<?php\n".'if ($_l->extends) { ob_start(); }'."\n".'elseif (isset($presenter, $control) && $presenter->isAjax()) { LatteMacros::renderSnippets($control, $_l, get_defined_vars()); }'."\n".'?>'.$s."<?php\n".'if ($_l->extends) { ob_end_clean(); LatteMacros::includeTemplate($_l->extends, get_defined_vars(), $template)->render(); }'."\n";}else{$s="<?php\n".'if (isset($presenter, $control) && $presenter->isAjax()) { LatteMacros::renderSnippets($control, $_l, get_defined_vars()); }'."\n".'?>'.$s;}if($this->namedBlocks){$uniq=$this->uniq;foreach(array_reverse($this->namedBlocks,TRUE)as$name=>$foo){$code=&$this->namedBlocks[$name];$namere=preg_quote($name,'#');$s=String::replace($s,"#{block $namere} \?>(.*)<\?php {/block $namere}#sU",callback(create_function('$matches','extract(NClosureFix::$vars['.NClosureFix::uses(array('name'=>$name,'code'=>&$code,'uniq'=>$uniq)).'], EXTR_REFS); 
...
String::replace($this->macros[$macro],'#%(.*?)%#',callback(create_function('$m','extract(NClosureFix::$vars['.NClosureFix::uses(array('This'=>$This,'content'=>$content,'modifiers'=>$modifiers)).'], EXTR_REFS); 
				if ($m[1]) {
...
new
InvalidStateException("Unknown syntax '$var' on line {$this->filter->line}.");}}function
...
new
InvalidStateException("Missing destination in {include} on line {$this->filter->line}.");}elseif($destination[0]==='#'){$destination=ltrim($destination,'#');if(!String::match($destination,'#^'.self::RE_IDENTIFIER.'$#')){throw
new
...
new
InvalidStateException("Cannot include $destination block outside of any block on line {$this->filter->line}.");}$destination=$item[1];}$name=var_export($destination,TRUE);$params.=$isDefinition?'get_defined_vars()':'$template->getParams()';$cmd=isset($this->namedBlocks[$destination])&&!$parent?"call_user_func(reset(\$_l->blocks[$name]), \$_l, $params)":'LatteMacros::callBlock'.($parent?'Parent':'')."(\$_l, $name, $params)";return$modifiers?"ob_start(); $cmd; echo ".self::formatModifiers('ob_get_clean()',$modifiers):$cmd;}else{$destination=self::formatString($destination);$cmd='LatteMacros::includeTemplate('.$destination.', '.$params.'$template->getParams(), $_l->templates['.var_export($this->uniq,TRUE).'])';return$modifiers?'echo '.self::formatModifiers($cmd.'->__toString(TRUE)',$modifiers):$cmd.'->render()';}}function
...
new
InvalidStateException("Multiple {extends} declarations are not allowed; on line {$this->filter->line}.");}$this->extends=$destination!=='none';return$this->extends?'$_l->extends = '.self::formatString($destination):'';}function
...
new
InvalidStateException("Tag {/block $content} was not expected here on line {$this->filter->line}.");}elseif($type===self::BLOCK_NAMED){return"{/block $name}";}else{return$modifiers===''?'':'echo '.self::formatModifiers('ob_get_clean()',$modifiers);}}function
macroSnippet($content){if(substr($content,0,1)===':'||!$this->oldSnippetMode){$this->oldSnippetMode=FALSE;return$this->macroBlock('_'.ltrim($content,':'),'');}$args=array('');if($snippet=self::fetchToken($content)){$args[]=self::formatString($snippet);}if($content){$args[]=self::formatString($content);}return'} if ($_l->foo = SnippetHelper::create($control'.implode(', ',$args).')) { $_l->snippets[] = $_l->foo';}function
macroSnippetEnd($content){if(!$this->oldSnippetMode){return$this->macroBlockEnd('','');}return'array_pop($_l->snippets)->finish(); } if (SnippetHelper::$outputAllowed) {';}function
...
new
InvalidStateException("Tag {/capture $content} was not expected here on line {$this->filter->line}.");}return$name.'='.self::formatModifiers('ob_get_clean()',$modifiers);}function
macroCache($content){return'if (CachingHelper::create('.var_export($this->uniq.':'.$this->cacheCounter++,TRUE).', $_l->g->caches'.self::formatArray($content,', ').')) {';}function
macroForeach($content){return'$iterator = $_l->its[] = new SmartCachingIterator('.preg_replace('# +as +#i',') as ',$content,1);}function
...
macroDump($content){return'Debug::barDump('.($content?'array('.var_export($content,TRUE)." => $content)":'get_defined_vars()').', "Template " . str_replace(dirname(dirname($template->getFile())), "\xE2\x80\xA6", $template->getFile()))';}function
...
new
InvalidStateException("Modifier name must be alphanumeric string, '$token' given.");}}else{if($token===':'||$token===','){$var=$var.', ';}elseif($token==='|'){$var=$var.')';$inside=FALSE;}elseif($name===self::T_SYMBOL&&$quote&&!is_numeric($token)&&in_array($tokenizer->nextToken($n),array(',',':',')','|',NULL),TRUE)){$var.="'$token'";}else{$var.=$token;}}if($name!==Tokenizer::T_WHITESPACE){$quote=in_array($token,array('[',',','=','(',':'));}}return$inside?"$var)":$var;}static
function
...
new
InvalidStateException("Callback '$textual' is not callable.");}$res=preg_replace_callback($pattern,$replacement,$subject,$limit);}elseif(is_array($pattern)){$res=preg_replace(array_keys($pattern),array_values($pattern),$subject,$limit);}else{$res=preg_replace($pattern,$replacement,$subject,$limit);}self::catchPregError($pattern);return$res;}static
function
...
getIterator(){return
new
ArrayIterator($this->tokens);}function
...
new
InvalidStateException('A session had already been started by session.auto-start or session_start().');}try{$this->configure($this->options);}catch(NotSupportedException$e){}Debug::tryError();session_start();if(Debug::catchError($msg)){@session_write_close();throw
new
